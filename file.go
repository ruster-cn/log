...
func (p *XFileLog)NewLogFromConfiguration(c *LogConfiguration)(err error){
	...
	p.service = c.Service
	isDir, err := isDir(c.FileConfig.Path)
	if err != nil || !isDir {
		err = os.MkdirAll(c.FileConfig.Path, 0755)
		if err != nil {
			return fmt.Errorf("Mkdir failed, err:%v", err)
		}
	}
	p.path = c.FileConfig.Path
	p.filename = c.FileConfig.Name
...
func (p *XFileLog) Spliter() {
	preDay := time.Now().Day()
	splitTime := time.Now().Format("20060102")
	defer p.Close()
	for {
		time.Sleep(time.Second * SpliterDelay)
		if time.Now().Day() != preDay {
			p.Clean()
			p.ReName(splitTime)
			preDay = time.Now().Day()
			splitTime = time.Now().Format("20060102")
